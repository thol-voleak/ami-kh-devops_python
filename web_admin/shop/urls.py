...
url(r'^ajax/agent-detail/(?P<id>[0-9A-Za-z]+)/$', login_required(Agent_Detail.as_view(), login_url='authentications:login'), name="ajax_agent_detail"),
    ...
    url(r'^(?P<agent_id>[0-9A-Za-z]+)/(?P<shop_id>[0-9A-Za-z]+)/channels/(?P<channel_id>[0-9A-Za-z]+)/grant$', login_required(grant_channel_access, login_url='authentications:login'), name="grant_channel_access"),
    url(r'^(?P<agent_id>[0-9A-Za-z]+)/(?P<shop_id>[0-9A-Za-z]+)/channels/(?P<channel_id>[0-9A-Za-z]+)/revoke$', login_required(revoke_channel_access, login_url='authentications:login'), name="revoke_channel_access"),
