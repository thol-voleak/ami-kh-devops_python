...
class RestFulClient:
    ...
    @classmethod
    def upload(cls, url, files, headers, loggers, params={}, timeout=None):
        if 'http' not in url:
            url = settings.DOMAIN_NAMES + url
        if timeout is None:
            timeout = settings.GLOBAL_TIMEOUT
        try:
            ...
            response = requests.post(url, files=files, headers=headers, data=params, verify=settings.CERT, timeout=timeout)
            ...
            try:
                ...
                if (http_status_code == 200):
                    ...
                    status_message = status.get('message', '')
                    is_success = True
                elif (http_status_code == 400 or http_status_code == 500):
                    status = response_json.get('status', {})
                    status_code = status.get('code', '')
                    status_message = status.get('message', '')
                else:
                    # TODO: case any error
                    loggers.error("Unknown error occurs")
                loggers.info('Http status code: [{status_code}].'.format(status_code=status_code))
                loggers.info('Status message: [{status_message}].'.format(status_message=status_message))

                data = response_json.get('data', '')

                is_success = (http_status_code == 200)
            except Exception as e:
                loggers.error(e)
                raise Exception(response)
        except requests.exceptions.Timeout:
