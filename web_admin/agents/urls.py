...
urlpatterns = [
    ...
    url(r'^(?P<agent_id>[0-9A-Za-z]+)/sofcash/$', login_required(SOFCashView.as_view(), login_url='authentications:login'), name="agent-sofcash"),
    url(r'^(?P<agent_id>[0-9A-Za-z]+)/sofbank/$', login_required(SOFBankView.as_view(), login_url='authentications:login'), name="agent-sofbank"),
